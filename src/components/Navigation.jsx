  ...
  return (
    <div className='bg-white max-h-[4.4rem] px-6 rounded-t-xl'>
      <ul className='flex relative'>
        <span className={`bg-color7 duration-500 ${Menus[active].dis} border-4 border-grey-900 h-16 w-16 absolute -top-5 rounded-full`}>
        ...
        {Menus.map((menu, i) => (
              <li key={i} className='w-16'>
                <a className='flex flex-col text-center pt-6' onClick={() => setActive(i)}>
                  <span className={`text-xl cursor-pointer duration-500 ${active === i && '-mt-6 text-white'}`}>
